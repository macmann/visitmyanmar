...
describe('data-driven locations and activities',()=>{
 ...
 it('supports the tea shop and landmark discovery loop',()=>{expect(locationById('tea_shop')?.activities).toEqual(expect.arrayContaining(['food_mohinga','tea_culture','meet_tea_owner','landmark_recommendation']));expect(locationById('shwedagon')?.activities).toContain('reveal_viewpoint');expect(activityById('reveal_viewpoint')?.discoveryId).toBe('viewpoint_revealed')});
});
